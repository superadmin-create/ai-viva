    ...
    await db.query(
      `INSERT INTO viva_results 
       ...
       ON CONFLICT (vapi_call_id) DO UPDATE SET
        student_name = CASE WHEN LENGTH(EXCLUDED.student_name) > 0 THEN EXCLUDED.student_name ELSE COALESCE(viva_results.student_name, EXCLUDED.student_name) END,
        student_email = CASE WHEN EXCLUDED.student_email != 'unknown@example.com' AND LENGTH(EXCLUDED.student_email) > 0 THEN EXCLUDED.student_email ELSE COALESCE(viva_results.student_email, EXCLUDED.student_email) END,
        subject = CASE WHEN LENGTH(EXCLUDED.subject) > 0 THEN EXCLUDED.subject ELSE COALESCE(viva_results.subject, EXCLUDED.subject) END,
        topics = CASE WHEN LENGTH(EXCLUDED.topics) > 0 THEN EXCLUDED.topics ELSE COALESCE(viva_results.topics, EXCLUDED.topics) END,
        questions_answered = CASE WHEN EXCLUDED.questions_answered > 0 THEN EXCLUDED.questions_answered ELSE COALESCE(viva_results.questions_answered, EXCLUDED.questions_answered) END,
        score = CASE WHEN EXCLUDED.score > 0 THEN EXCLUDED.score ELSE COALESCE(viva_results.score, EXCLUDED.score) END,
        overall_feedback = CASE WHEN LENGTH(EXCLUDED.overall_feedback) > 0 THEN EXCLUDED.overall_feedback ELSE COALESCE(viva_results.overall_feedback, EXCLUDED.overall_feedback) END,
        transcript = CASE WHEN LENGTH(EXCLUDED.transcript) > LENGTH(COALESCE(viva_results.transcript, '')) THEN EXCLUDED.transcript ELSE COALESCE(viva_results.transcript, EXCLUDED.transcript) END,
        recording_url = CASE WHEN LENGTH(EXCLUDED.recording_url) > 0 THEN EXCLUDED.recording_url ELSE COALESCE(viva_results.recording_url, EXCLUDED.recording_url) END,
        evaluation = CASE WHEN EXCLUDED.evaluation IS NOT NULL AND EXCLUDED.evaluation::text != '{}' AND EXCLUDED.evaluation::text != 'null' THEN EXCLUDED.evaluation ELSE COALESCE(viva_results.evaluation, EXCLUDED.evaluation) END,
        teacher_email = CASE WHEN LENGTH(EXCLUDED.teacher_email) > 0 AND EXCLUDED.teacher_email != 'unknown@example.com' THEN EXCLUDED.teacher_email ELSE COALESCE(viva_results.teacher_email, EXCLUDED.teacher_email) END,
        marks_breakdown = CASE WHEN jsonb_typeof(EXCLUDED.marks_breakdown) = 'array' AND jsonb_array_length(EXCLUDED.marks_breakdown) > 0 THEN EXCLUDED.marks_breakdown ELSE COALESCE(viva_results.marks_breakdown, EXCLUDED.marks_breakdown) END`,
      [
        data.timestamp,
        data.student_name,
        studentEmail,
